...
LITECOIND=${LITECOIND:-$SRCDIR/defiprofitcoind}
LITECOINCLI=${LITECOINCLI:-$SRCDIR/defiprofitcoin-cli}
LITECOINTX=${LITECOINTX:-$SRCDIR/defiprofitcoin-tx}
LITECOINQT=${LITECOINQT:-$SRCDIR/qt/defiprofitcoin-qt}

[ ! -x $LITECOIND ] && echo "$LITECOIND not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
DPCVER=($($LITECOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
