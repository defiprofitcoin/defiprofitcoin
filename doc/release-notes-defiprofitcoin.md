Defiprofitcoin Core version *0.15.1* is now available from:

  <https://download.defiprofitcoin.com/defiprofitcoin-0.15.1/>
...
A number of changes to the way Defiprofitcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 mDPCC per kB (0.00001 DPC). This is to prepare for dropping the minimum transaction fee to 0.1 mDPCC per kB (0.0001 DPC) in the next release.
